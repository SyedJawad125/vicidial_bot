...
FIXES applied:
  - UnicodeEncodeError on Windows (cp1252)  → replaced X/arrow chars with ASCII
  - /tmp warmup path on Windows             → use tempfile.gettempdir()
  - Windows logging handler                 → UTF-8 encoding forced on FileHandler
"""
...
FASTAGI_HOST  = getattr(settings, "FASTAGI_HOST", "127.0.0.1")
FASTAGI_PORT  = getattr(settings, "FASTAGI_PORT", 4577)
MAX_WORKERS   = getattr(settings, "FASTAGI_MAX_WORKERS", 20)
LANGUAGE      = getattr(settings, "BOT_LANGUAGE", "en")
...
class FastAGIRequestHandler(socketserver.StreamRequestHandler):
    ...
    def handle(self):
        try:
            ...
            script_name = agi.env.get("agi_network_script", "voice_bot").strip("/")
            log.info("Script: %s  |  caller: %s", script_name, agi.caller_id)
...
class SocketAGI:
    """AGI interface over a TCP socket (FastAGI mode)."""
    ...
    def answer(self)                          -> dict: return self._send("ANSWER")
    def hangup(self)                          -> dict: return self._send("HANGUP")
    def stream_file(self, f, esc="")          -> dict: return self._send(f'STREAM FILE "{f}" "{esc}"')
    def set_variable(self, n, v)              -> dict: return self._send(f'SET VARIABLE {n} "{v}"')

    def record_file(self, filename, fmt="wav",
                    escape_digits="#", timeout_ms=15000,
                    offset=0, beep=False, silence_sec=2) -> dict:
        beep_s    = "BEEP" if beep else ""
        silence_s = f"s={silence_sec}"
        return self._send(
            f'RECORD FILE "{filename}" {fmt} "{escape_digits}" '
            f"{timeout_ms} {offset} {beep_s} {silence_s}"
        )
...
def preload_engines(language: str = "en") -> None:
    """
    Load Vosk + MeloTTS into RAM at startup so first caller gets instant response.
    Uses ASCII-only log messages to avoid Windows cp1252 UnicodeEncodeError.
    """
    log.info("Pre-loading voice engines (language=%s)...", language)
    start = time.monotonic()
    ...
    try:
        stt = get_stt_engine(language)
        log.info("  [OK] Vosk STT ready  (%.1fs)", time.monotonic() - start)
    except FileNotFoundError as exc:
        log.error("  [FAIL] Vosk model not found.\n%s", exc)
        log.error(
            "  To fix: download the model and place it at: %s",
            getattr(settings, "VOSK_MODELS", {}).get(language, f"vosk_models/{language}")
        )
        log.error("  Windows download steps are in SETUP_GUIDE.txt — Section 3B")
    except Exception as exc:
        log.error("  [FAIL] Vosk STT failed: %s", exc)

    # ── TTS ───────────────────────────────────────────────────────────────────
    try:
        tts = get_tts_engine(language)

        # Warm-up: generate a short phrase so first call is instant
        # Use OS temp dir (works on both Windows and Linux)
        warmup_path = os.path.join(tempfile.gettempdir(), "_agi_warmup.wav")
        tts.synthesize("System ready.", output_path=warmup_path)
        try:
            os.unlink(warmup_path)
        except OSError:
            pass

        log.info("  [OK] MeloTTS ready  (%.1fs)", time.monotonic() - start)
    except Exception as exc:
        log.error("  [FAIL] MeloTTS TTS failed: %s", exc)
...
def _setup_logging():
    """
    Configure logging with UTF-8 encoding on all handlers.
    This prevents UnicodeEncodeError on Windows (cp1252 default codec).
    """
    Path("logs").mkdir(exist_ok=True)
    ...
    file_handler = logging.FileHandler(
        "logs/fastagi.log", mode="a", encoding="utf-8"
    )
...
def _test_engines():
    ...
    try:
        stt   = get_stt_engine("en")
        ready = stt.is_ready()
        print(f"    [OK] Vosk model loaded: {ready}")
    except Exception as exc:
