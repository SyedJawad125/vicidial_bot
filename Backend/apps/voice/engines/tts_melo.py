...
FIXES applied:
  - 'HParams' object has no attribute 'get'  → convert spk2id to plain dict
  - Windows /tmp path                        → use tempfile.gettempdir()
  - Unicode log chars                        → ASCII safe symbols only
"""
...
# ── Config ────────────────────────────────────────────────────────────────────
TTS_AUDIO_DIR: Path = Path(
    getattr(settings, "VOICE_AUDIO_DIR",
            Path(settings.BASE_DIR) / "voice_audio" / "tts")
)
TTS_AUDIO_DIR.mkdir(parents=True, exist_ok=True)
...
class MeloTTS:
    ...
    Usage:
        tts = MeloTTS(language="en")
        path = tts.synthesize("Hello, how are you today?")
        # Returns path to 16 kHz mono WAV
    """

    def __init__(self, language: str = "en"):
        self.language     = language.lower()
        self._model       = None
        self._speaker_ids = {}   # always a plain dict after _load_model()
        self._load_model()
    ...
    def _load_model(self) -> None:
        ...
        try:
            ...
            self._model       = _MeloTTS(language=melo_lang, device=device)
            self._speaker_ids = self._extract_speaker_ids()

            log.info("MeloTTS ready (%s). Available speakers: %s",
                     melo_lang, list(self._speaker_ids.keys()))
    ...
    def _resolve_melo_lang(self) -> str:
        ...
        resolved = mapping.get(self.language, "EN")
        log.debug("Language '%s' maps to MeloTTS '%s'", self.language, resolved)
        return resolved

    def _extract_speaker_ids(self) -> dict:
        """
        Convert MeloTTS spk2id (HParams or dict) to a plain Python dict.

        MeloTTS versions differ:
          - Older  → hps.data.spk2id is a plain dict
          - Newer  → hps.data.spk2id is an HParams object (no .get())

        We normalise both to a plain dict here so the rest of the code
        can safely use dict.get() / dict[key].
        """
        raw = self._model.hps.data.spk2id

        # Already a plain dict
        if isinstance(raw, dict):
            log.debug("spk2id is already a dict: %s", raw)
            return raw

        # HParams / AttrDict — convert via __dict__
        if hasattr(raw, "__dict__"):
            converted = {k: v for k, v in vars(raw).items()
                         if not k.startswith("_")}
            log.debug("Converted HParams spk2id to dict: %s", converted)
            return converted

        # Last resort — try dict()
        try:
            converted = dict(raw)
            log.debug("Converted spk2id via dict(): %s", converted)
            return converted
        except Exception as exc:
            log.warning("Could not convert spk2id (%s): %s — using fallback", type(raw), exc)
            return {"EN-Default": 0}
    ...
    def synthesize(self, text: str, output_path: Optional[str] = None) -> str:
        """
        Convert text to speech and save as 16 kHz mono WAV.
    ...
    def _synthesize_melo(self, text: str, output_path: str) -> str:
        """Generate audio with MeloTTS, then resample to 16 kHz mono."""
        cfg        = MELO_SPEAKERS.get(self.language, MELO_SPEAKERS["en"])
        speaker    = cfg["speaker"]
        speed      = cfg["speed"]

        # Resolve speaker ID from our plain-dict copy
        speaker_id = self._speaker_ids.get(speaker)

        if speaker_id is None:
            # Try case-insensitive match
            for k, v in self._speaker_ids.items():
                if k.upper() == speaker.upper():
                    speaker_id = v
                    log.debug("Speaker matched case-insensitive: %s -> %s", speaker, k)
                    break

        if speaker_id is None:
            # Fall back to whichever speaker is available
            if self._speaker_ids:
                first_key  = next(iter(self._speaker_ids))
                speaker_id = self._speaker_ids[first_key]
                log.warning(
                    "Speaker '%s' not found in %s — using '%s' (id=%s)",
                    speaker, list(self._speaker_ids.keys()), first_key, speaker_id
                )
            else:
                speaker_id = 0
                log.warning("No speakers found — defaulting to id=0")

        # MeloTTS writes a WAV, we then resample to 16 kHz for Asterisk
    ...
    def _synthesize_gtts(self, text: str, output_path: str) -> str:
        """Fallback: gTTS for languages MeloTTS does not support."""
        try:
            from gtts import gTTS
    ...
    @staticmethod
    def _resample_to_asterisk(src_wav: str, dst_wav: str) -> None:
        """Resample any WAV to 16 kHz 16-bit mono (Asterisk / Vosk format)."""
        try:
            from pydub import AudioSegment
            audio = (
                AudioSegment.from_wav(src_wav)
            ...
            import shutil
            shutil.copy(src_wav, dst_wav)
            log.warning("pydub not installed — WAV not resampled. Run: pip install pydub")
